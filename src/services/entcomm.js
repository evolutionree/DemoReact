...
export async function getAdvanceSearchProtocol(entityid) {
  ...
  return Promise.all([
    ...
    queryListFilter(entityid)
  ]).then(([result1, result2]) => {
    const allFields = result1.data.entityfieldpros;
    const searchFields = result2.data.fieldssearch
      .map(field => formatField(field, allFields))
      .filter(field => !!field);
...
export async function queryPlugins({ entityid, recid }) {
  function queryDynamicEntities() {
    ...
    return request('/api/EntityPro/queryentitypro', params)
      .then(result => result.data.pagedata.filter(item => item.relentityid === entityid));
  };
  ...
  function queryPluginVisible() {
    ...
    return request('/api/dynamicentity/pluginvisible', params)
  ...
  return Promise.all([
        ...
        const plugins = visibleEntities.map(entity => {
          const plugin = {
            type: 'normal',
            entity
          };
