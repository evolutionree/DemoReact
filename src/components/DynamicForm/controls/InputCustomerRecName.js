...
class InputCustomerRecName extends Component {
  ...
  getEntityDetail = () => {
    ...
    if (entityId) {
      queryEntityDetail(entityId).then(res => {
        ...
        if (
          typeof data === 'object' &&
          Array.isArray(data.entityproinfo) &&
          data.entityproinfo.length &&
          data.entityproinfo[0].servicesjson
        ) {
          const servicesjson = data.entityproinfo[0].servicesjson
          const setreference =
            servicesjson.entityConfig &&
            JSON.parse(servicesjson.entityConfig).DisableRef
  ...
  render () {
    ...
    return (
      <div
        className={styles.InputCustomerRecNameWrap}
        id='InputCustomerRecNameWrap'
      >
        <Input
        ...
        {mode !== 'EDIT' ? (
          <div
            style={{ display: listHide ? 'none' : 'block' }}
          ...
          >
            ...
            {this.state.repeatCustomData.map((item, index) => {
              return (
                <ul key={item.recid}>
                  <li>
                    <span className={styles.spanLeft} title={item.recname}>
                      {item.recname}
                    </span>
                    {this.getEntityInfo().showQuote && setreference === '0' && (
                      <span
                        className={styles.spanRight}
                        onClick={this.quoteCustomer.bind(this, item)}
                      >
                        引用
                      </span>
                    )}
