...
export default class FilterModal extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (isOpening) {
      ...
      const fieldname = ColumnFilter && Object.keys(ColumnFilter).map(item => item);
      if (Array.isArray(fieldname)) this.setState({ selectValueList: fieldname, ColumnFilter });
    } else if (isClosing) {
  ...
  render() {
    ...
    const defaultProtocol = protocol && protocol.filter(field => (!has_No_Filter_Field.includes(field.controltype)));
    ...
    return (
      ...
      >
        <div>
          ...
          >
            {
              defaultProtocol.map(item => <Option key={item.fieldname}>{`${item.displayname}`}</Option>)
            }
          </Select>
          {
