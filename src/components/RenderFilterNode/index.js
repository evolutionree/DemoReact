...
const RenderFilterNode = props => {
  ...
  const getTransformDateValue = toTransformValue => {
    if (typeof toTransformValue === 'string' && toTransformValue !== '') {
      ...
      if (
        filterShowComponent.multiple.indexOf(controltype) === -1 &&
        toTransformValue === 'isnull'
      ) { return null }
      ...
      if (filterShowComponent.date.indexOf(controltype) > -1) {
        ...
        filterShowComponent.multiple.indexOf(controltype) > -1 ||
        filterShowComponent.number.indexOf(controltype) > -1
      ) {
        newValue =
          newValue &&
  ...
  const formatStr = e => {
    ...
    if (filterShowComponent.date.indexOf(controltype) > -1) {
      if (formatValue && formatValue.length === 2) {
        return [
          formatValue[0].format(dateFormat) + ' 00:00:00',
          formatValue[1].format(dateFormat) + ' 23:59:59'
        ].join(',')
      }
      return []
    }
  ...
  const renderComponent = () => {
    if (filterShowComponent.date.indexOf(controltype) > -1) {
      ...
      return (
        <RangePicker
          ...
          allowClear={false}
          onChange={e => onHandleValue(formatStr(e), fieldname)}
          value={text || null}
          format={dateFormat}
        />
      )
    ...
    if (filterShowComponent.single.indexOf(controltype) > -1) {
      // 实体类型
      return (
        <SingleSelect
          width={240}
    ...
    if (filterShowComponent.multiple.indexOf(controltype) > -1) {
      // 字典
    ...
    if (filterShowComponent.number.indexOf(controltype) > -1) {
      // 数字筛选
      return (
        <RangeNumber
          width={240}
  ...
  return (
    <div className={styles.wrap}>
      <span title={title} className={styles.labelName}>
        <span>{title}</span>
        <font>：</font>
      </span>
      {renderComponent()}
