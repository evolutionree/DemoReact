...
class SetDynamicFieldsModal extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (isOpening) {
      ...
      queryDynamicConfig(params).then(result => {
        let fieldids;
  ...
  fetchEntityType = () => {
    queryTypes({ entityId: this.props.entityId }).then(result => {
      const entityTypeId = result.data.entitytypepros[0].categoryid;
      ...
      >
        <Form>
          <FormItem label="显示字段一">
            {getFieldDecorator('fieldids[0]', {
            })(
              this.renderFieldSelect()
            )}
          </FormItem>
          <FormItem label="显示字段二">
            {getFieldDecorator('fieldids[1]', {
            })(
              this.renderFieldSelect()
            )}
          </FormItem>
          <FormItem label="显示字段三">
            {getFieldDecorator('fieldids[2]', {
            })(
              this.renderFieldSelect()
            )}
          </FormItem>
          <FormItem label="显示字段四">
            {getFieldDecorator('fieldids[3]', {
            })(
              this.renderFieldSelect()
            )}
          </FormItem>
          <FormItem label="显示字段五">
            {getFieldDecorator('fieldids[4]', {
            })(
              this.renderFieldSelect()
            )}
          </FormItem>
