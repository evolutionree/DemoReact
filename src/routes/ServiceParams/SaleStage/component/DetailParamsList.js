...
import ParamEditable from './DetailParamEditable';
import styles from './styles.less';

class DetailParamsList extends React.Component {
  static propTypes = {
      ...
      <li className={styles.row} key={index}>
        ...
        {this.props.fields.map(field => (
          <ParamEditable
            ...
            icon={isEditing ? this.state.currFieldsValues.isneedupfile : item.isneedupfile === 1 ? 'file' : ''}
          />
        ))}
        ...
        <div className={styles.actions}>
          {onOrderUp ? <Icon
            type="arrow-up"
            style={isFirst ? hidden : visible}
            onClick={() => { onOrderUp(item, index); }}
          /> : ''}
          {onOrderDown ? <Icon
            type="arrow-down"
            style={isLast ? hidden : visible}
...
export default DetailParamsList;
