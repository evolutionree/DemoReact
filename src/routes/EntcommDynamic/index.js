  ...
  return (
    <Page title={entityName}>
      <Toolbar>
        ...
        <Toolbar.Right>
          <Search placeholder={`请输入${searchTips || '关键字'}`} value={keyword} onSearch={val => searchKeyword(val)}>
            搜索
          </Search>
          <Button onClick={cancelFilter} style={{ marginLeft: '10px', height: '31px' }}>
            筛选
          </Button>
          <Icon
            type='setting'
            onClick={openSetHeader}
            style={{
              fontSize: '20px',
              marginLeft: '10px',
              cursor: 'pointer',
