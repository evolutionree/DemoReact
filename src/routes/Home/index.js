...
class Home extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      width: this.props.siderFold ? document.body.clientWidth - 61 : document.body.clientWidth - 200, //系统左侧 200px显示菜单
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      width: nextProps.siderFold ? document.body.clientWidth - 61 : document.body.clientWidth - 200
    });
  }
  ...
  queryReportDefine() {
    request('/api/ReportEngine/mainpagereport', {
      ...
      result.data.datasources && result.data.datasources instanceof Array && result.data.datasources.map((item) => {
        this.setState({
  ...
  renderComponentItem(width, component) {
    const chartHeight = component.height > 0 ? component.height : width * Math.abs(component.height);
    const style = { width: '100%', height: chartHeight, background: '#242935' };
    switch (component.reportiteminfo.commoncomponentinfo && component.reportiteminfo.commoncomponentinfo.elemtype) {
      //柱状图、折线图
      case 1:
        return (
          <div style={{ ...style }}>
            <ShowChart chartType="barOrline"
                       loading={this.state[component.cellid + 'loading']}
                       component={component.reportiteminfo.commoncomponentinfo.barandlineinfo}
        ...
        return (
          <div style={{ ...style }}>
            <ShowChart chartType="scatter"
                       loading={this.state[component.cellid + 'loading']}
                       component={component.reportiteminfo.commoncomponentinfo.scatterinfo}
                  ...
                  chartData.map((chartItem, chartIndex) => {
                    return (
                      <div key={chartIndex} style={{ width: '100%', height: '100%' }}>
                        <ShowFunnelChart
                                         loading={this.state[component.cellid + 'loading']}
                                         component={component.reportiteminfo.commoncomponentinfo.funelinfo}
      ...
      case 6:
        return (
          <div style={{ ...style, height: 120, background: 'transparent' }}>
            <Card component={component.reportiteminfo.commoncomponentinfo.diveleminfo} dataSource={this.state[component.cellid]} />
          </div>
        );
      case 7:
        return (
          <div style={{ ...style }}>
            <List component={component.reportiteminfo.commoncomponentinfo.simplelistinfo} dataSource={this.state[component.cellid]} totalHeight={chartHeight} />
          </div>
        )
      default:
        return null;
  ...
  render() {
    ...
    for (let i = 0; i < columnWithArray.length; i++) {
      if (columnWithArray[i] === 0) {
        ...
        if (this.state.columnsinfo && this.state.columnsinfo instanceof Array) {
          columnWithArray[i] = columnWithArray[i] - 20 - (this.state.columnsinfo.length * 10);
        }
      }
    }
    ...
    return (
      <div className={cls} style={{ width: '100%', height: 'calc(100vh - 60px)' }}>
        {
          this.state.columnsinfo && this.state.columnsinfo instanceof Array && this.state.columnsinfo.map((item, index) => {
            return (
              <div key={index} style={{ width: columnWithArray[index] + 10, float: 'left' }}>
                {
                  item.cellitems && item.cellitems instanceof Array && item.cellitems.map((component, componentIndex) => {
                    if (component.reportiteminfo && component.reportiteminfo.componenttype === 1) {
                      const chartHeight = component.height > 0 ? component.height : width * Math.abs(component.height);
                      const style = { width: '100%', height: chartHeight };
                      return (
                        <div key={componentIndex} className={Styles.chartWrap}>
                          {
                            component.isdisplaytitle ? <div className={Styles.normalChartTitle}>{component.title}</div> : null
                          }
                    ...
                    } else if (component.reportiteminfo && component.reportiteminfo.componenttype === 2) {
                      return <iframe src={component.reportiteminfo.iframeinfo.url} width="100%" height="100%" style={{ border: 'none' }}></iframe>;
                    } else if (component.reportiteminfo && component.reportiteminfo.componenttype === 3) {
                      return (
                        <div key={componentIndex} className={Styles.chartWrap}>
                          {
                            component.isdisplaytitle ? <div className={Styles.normalChartTitle}>{component.title}</div> : null
                          ...
                          {
                            this.renderComponentItem(columnWithArray[index], component)
                          }
                        </div>
                      );
                    }
