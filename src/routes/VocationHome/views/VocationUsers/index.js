...
function VocationUsers({
  ...
  function handleDel() {
    Modal.confirm({
      title: '确定移除选中用户吗',
      onOk() {
        const ids = _.map(currentRecords, 'userid');
        dispatch({ type: 'vocationUsers/del', payload: ids });
      }
    });
  }
  function handleSelect(items) {
    dispatch({ type: 'vocationUsers/currentRecords', payload: items });
  }
  ...
  return (
    <div>
      ...
      >
        ...
        <Column title="拥有职能" key="vocationname" dataIndex="vocationname" />
      </Table>
    </div>
  );
}
