...
class FunctionRuleModal extends Component {
  ...
  fetchFunctionRules = () => {
    ...
    const params = { entityid, functionid: funcid, vocationid: this.props.vocationId };
 ...
 */
  handleSubmit = () => {
    ...
    const { vocationId, currFunction } = this.props;
...
export default connect(
  state => {
    const { currFunction, vocationId } = state.vocationRule;
    return {
      vocationId,
      currFunction,
      visible: !!currFunction
    };
  },
  dispatch => {
    return {
      close() {
        dispatch({ type: 'vocationRule/putState', payload: { currFunction: null } });
      }
    };
  }
)(FunctionRuleModal);
