...
class AdvanceSearchModal extends Component {
  ...
  render() {
    const { visible, submit, cancel, entityId } = this.props;
...
export default connect(
  state => {
    const { showModals, relEntityId, entityName } = state.entcommRel;
    return {
  ...
  dispatch => {
    return {
      ...
      submit(formData) {
        dispatch({ type: 'entcommRel/advanceSearch', payload: formData });
        dispatch({ type: 'entcommRel/showModals', payload: '' });
      }
    };
  }
