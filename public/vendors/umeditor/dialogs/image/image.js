...
//                }
              ...
              var fileId = node.src.split('fileid=')[1] && node.src.split('fileid=')[1];
              var src = editor.options.x_img_base64 ? (getBase64Image(node) || node.src) : node.src;
              return arr.push({
                _src: src,
                src: src,
                alt: fileId,
        ...
        callback: function (editor, $w, url, state) {
            ...
            if (state == "SUCCESS") {
                //显示图片计数+1
                Upload.showCount++;
                var $img = $("<img src='" + '/api/fileservice/read?fileid='+ url + "' class='edui-image-pic' />"),
    ...
    var Upload = {
        ...
        uploadComplete: function(r){
            ...
            try{
                var json = eval('('+r+')');
                Base.callback(me.editor, me.dialog, json.data, 'SUCCESS');
            }catch (e){
        ...
        submit: function (callback) {
            ...
            $(me.dialog).delegate( ".edui-image-file", "change", function ( e ) {
                ...
                var FileController = '/api/fileservice/upload';
                // FormData 对象
                ...
                xhr.setRequestHeader('Authorization', 'Bearer ' + window.UMEDITOR_UPLOAD_TOKEN);
            ...
            'ok': {
                exec: function (editor, $w) {
                    ...
                    list.forEach(function(item) {
                      // 去掉host
                      if (item.src) {
                        item.src = item.src.replace(/(^\S*)(\/api\/fileservice\S+$)/, '$2');
                      }
                    });
